...
void init_input(){
	...
	for(int i = 0; i < line.size(); i ++){
		...
		for(int j = 0; j < line[i].station.size(); j ++){
			...
			station_first_time[a * line_num + i] = ft + j * run_time;
		}
	}
}
	...
	for(int i = 0; i < line.size(); i ++){
		...
		for(int j = 0; j < line[i].station.size(); j ++){
			...
			for(int k = 0; k < line.size(); k ++){
				if(k != i && station_first_time[a * line_num + k] != -1){
					if(station_first_time[a * line_num + i] >= station_first_time[a * line_num + k]){
						int wait_time = station_first_time[a * line_num + i] - station_first_time[a * line_num + k];
						addEdge(a * line_num + k, a * line_num + i, wait_time, yuan);
					}
					else{
						int next_time = 0;
						while(station_first_time[a * line_num + i] + next_time < station_first_time[a * line_num + k]){
							next_time += delta;
						}
						int wait_time = station_first_time[a * line_num + i] + next_time - station_first_time[a * line_num + k];
						addEdge(a * line_num + k, a * line_num + i, wait_time, yuan);
					}
				}
			}
			...
			if(j){
				...
				if(yuan == 3){
					addEdge(st * line_num + i, ed * line_num + i, 5, 0);
				}
				else{
					addEdge(st * line_num + i, ed * line_num + i, 3, 0);
				}
			}
		}
	}
...
void make_st_graph(string st){
	...
	for(int i = 0; i < line_num; i ++){
		int yuan = line[i].yuan;
		int wait_time = station_first_time[a * line_num + i];
